...
# sys.path.append(os.path.join(BASE_DIR, "lib")) # HACK add the lib folder
from .config import CONF
# from utils.pc_utils import random_sampling, rotx, roty, rotz
# print(sys.path, ' << sys.path from dataset codes')
from .scanrefer_utils_pc_utils import random_sampling, rotx, roty, rotz
from .model_util_scannet import rotate_aligned_boxes, ScannetDatasetConfig, rotate_aligned_boxes_along_axis
...
class ScannetReferenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(point_cloud.shape, self.num_points, ' << pc shape', flush=True)
        ...
        # print(data_dict.keys(), '<< scanrefer data dict keys, TODO', flush=True)
        # print(num_bbox, '<< num-bbox', flush=True)
...
def get_scanrefer(scanrefer_train, scanrefer_val, num_scenes, no_reference):
    if no_reference:
        train_scene_list = get_scannet_scene_list("train")
        new_scanrefer_train = []
        for scene_id in train_scene_list:
            # 只用第一个refer的数据，就是乱用一个refer
...
def get_scanrefer_testset(args):
...
scanrefer = None
scanreferTest = None
def GetScanReferDataset(args, split):
    global scanrefer, all_scene_list
    global scanreferTest
    if split in ['train', 'val']:
        if scanrefer is None:
            print('Scanrefer-Dataset: Getting Scanrefer Dataset Path!')
            SCANREFER_TRAIN = json.load(open(os.path.join(CONF.PATH.DATA, "ScanRefer_filtered_train.json")))
            SCANREFER_VAL = json.load(open(os.path.join(CONF.PATH.DATA, "ScanRefer_filtered_val.json")))
            scanrefer_train, scanrefer_val, all_scene_list = get_scanrefer(SCANREFER_TRAIN, SCANREFER_VAL, args.get('num_scenes', -1), args.get('no_reference', False))
            scanrefer = {
                'train': scanrefer_train,
                'val': scanrefer_val,
            }
        list_scanrefer = scanrefer[split]
    elif split in ['test']:
        if scanreferTest is None:
            SCANREFER_TEST = json.load(open(os.path.join(CONF.PATH.DATA, "ScanRefer_filtered_test.json")))
            scanreferTest, test_scene_list = get_scanrefer_testset(args)
        list_scanrefer = scanreferTest[split]
    else:
        raise NotImplementedError('ScanRefer Dataset Name Split !=', split)
    dataset = ScannetReferenceDataset(
        scanrefer=list_scanrefer,
        scanrefer_all_scene=all_scene_list, 
        split=split, 
        num_points=args.get('num_points', 20000),
        use_height=(not args.get('no_height', False)),
        use_color=args.get('use_color', False),
        use_normal=args.get('use_normal', False),
        use_multiview=args.get('use_multiview', False),
    )
    return dataset
